...
- **2026-05-29:** bumped `7.0.0-rc10` → `7.0.0-rc13` (Critical CVE-2026-48063, fixed upstream in rc12; + rc11/13 protocol fixes). All three patched source files were untouched rc10→rc13, so the patch re-applied byte-identically (only the filename changed). Procedure: `docs/UPSTREAM_SYNC.md` → "Baileys-only version bump". Worked example: `docs/BAILEYS_UPGRADE_rc10_to_rc13.md`.
  - **Deployed to prod** 2026-05-29 ~15:40 UTC (commit `2ad1d5ce`). Current prod digest: `ghcr.io/brunubarbosa/evolution-api@sha256:61ee881d06a913aca0c7095e3c16f7694cfa515de2dd27d34222a323b6525ad0` (tag `gdw-v0.0.0-2ad1d5ce`).
  - **Rollback target (previous prod, rc10):** `ghcr.io/brunubarbosa/evolution-api@sha256:b6c5c6551a0b803f3c293a4bc05ca8aab1bf824620924db26e7f1bff6847541c`. To roll back: SSH `root@91.98.196.152`, `cd /data/coolify/services/gdg2okn1nxvpd1tybsjnbj50/`, restore the backed-up compose (`/tmp/compose-bak-rc13-*.yml`) or `sed -i 's|61ee881d…|b6c5c655…|' docker-compose.yml`, then `docker compose up -d`. Postgres/Redis volumes persist; instances reconnect from saved auth state (no QR).
  - **Verification:** internal `/` → `{status:200, version:"2.3.7"}`, baileys `7.0.0-rc13`, all 5 patch hunks present in the running image. The 5 cred-bearing instances reconnected to `open`. NOTE: a restart of this service triggers a transient burst of large batch webhooks (roster/history fan-out) that the worker rejects with `413 FST_ERR_CTP_BODY_TOO_LARGE` for ~3-4 min, then subsides — restart side-effect, not version-specific. Of the ~61 instances only ~9 have saved sessions and ~5 are ever live; the rest churn `connecting` permanently regardless of version.
